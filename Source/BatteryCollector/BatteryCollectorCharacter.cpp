...
ABatteryCollectorCharacter::ABatteryCollectorCharacter()
{
	...
	CollectionSphere = CreateDefaultSubobject<USphereComponent>(TEXT("CollectionShere"));
	CollectionSphere->AttachTo(RootComponent);
	CollectionSphere->SetSphereRadius(200.f);

	// set a base power level for the character
	IntialPower = 2000.f;
	CharacterPower = IntialPower;
...
{
	// get all overlaping actors and store in an array
	...
	for (int32 iCollected = 0; iCollected < CollectedActors.Num(); ++iCollected)
	{
		...
		{
			// call the pickups WasColleceted function
			TestPickup->WasCollected();
			//Chack to see if pickup is also a batter
	...
	// check to see if power needs to be added and if so add it
	if (CollectedPower > 0)
	{
...
float ABatteryCollectorCharacter::GetIntialPower()
{
	return IntialPower;
}
...
// Callaed whenever power is increased or decreased
